...
| # | Checkbox | Status |
|---|---|---|
| 1 | Router between two local Ollama models on a rule I wrote (not 50/50), reason logged per request | code ✓ · live pair pending model pull |
| 2 | Prometheus metrics + Grafana dashboard (5 panels) | code ✓ · k6/Grafana run pending |
| 3 | MCP server with `list_models`, `get_stats` (+3 more) | ✓ live handshake · recording pending |
| 4 | Fresh RAG app + reviewed golden set + hand-written faithfulness scorer | **✓ frozen v1, faith 0.969 on 2 live runs** |
...
38 tests, `go vet` clean. Full history, decisions (ADR-0001…0006) and every defect found by the review passes are in [`AgentOps_Project_Plan.md`](AgentOps_Project_Plan.md).
...
| `--draft-golden` → review → `--freeze-golden` | build the golden set (LLM drafts, human reviews, hash frozen) |
...
corpus/      15 short docs — the RAG target
dashboard/   grafana/router.json (5 panels)
