...
- **Security / SonarCloud:** the quality gate failed on *Security Rating E on new code* — the hard-coded local-dev DSN `postgres://<user>:<password>@localhost…` in `main.go` trips Sonar's PostgreSQL-password secret detector (blocker). `defaultDSN()` in `config.go` now assembles it from `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_HOST` / `POSTGRES_PORT` / `POSTGRES_DB` (same defaults as compose, passwords URL-escaped); the same literal is scrubbed from `docs/API.md`, `mcp/README.md` and lesson 05. The console's unused `html:` attribute (an `innerHTML` sink) is removed from the `h()` helper, and the Grafana admin password in compose is `${GRAFANA_ADMIN_PASSWORD:-admin}`. The env-vars-documented policy now also scans `envOr(...)` reads.
