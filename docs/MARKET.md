...
2. **Retrieval: the first vector-only misses.** Golden v3 (72 pairs, 36 chunks) scored faithfulness 0.972 with recall 0.972 — the two zeros (pair 12, "what error code when all candidate backends fail"; pair 66, "how does the health prober handle backend checks") are both **recall 0**: the source chunk was not in the top 5, so a true answer was judged against the wrong context. Every pair whose chunk was retrieved scored 1.000. At 36 chunks vector-only search already loses on vocabulary overlap; every competitor with RAG evals ships hybrid retrieval. **P0: BM25 + RRF (the parked GED weights), and report "retrieval miss" separately from "unfaithful".**
